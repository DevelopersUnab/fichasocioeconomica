...
var obtener_data_editar = function(tbody, dataTable) {
    $(tbody).on("click", "button.editar", function() {
        ...
        var txtcodigo = $('#txtcodigo').val(data.CODIGOPARAMETRO);
...
var obtener_data_eliminar = function(tbody, dataTable) {
    $(tbody).on("click", "button.eliminar", function() {
        var data = dataTable.row($(this).parents("tr")).data();        
        var txtcodigoE = $('#txtcodigoE').val(data.CODIGOPARAMETRO);
...
$(document).on('click', '#btnEliminarE', function() {
        ...
        urlbase = $("body").attr('urlbase');
        url = urlbase + "/configuracion/eliminarParametros";
        $.ajax({
            url: url,
            type: 'POST',
            dataType: 'json',
            ...
            if (data.success == true) {
                $("#alertSuccess").show(0).delay(4000).hide(0);
                $("#mensajeSuccess").text(data.message + ' "' + codigo + '".');
                $('#myModalEliminar').modal('hide');
                var dataTable = $('#res-config').DataTable();
                dataTable.ajax.reload();
            }
...
$(document).on('submit', '#user_form', function(event) {
        ...
        if (txtnombre == '') {
            $("#alertnombre").show(0).delay(3000).hide(0);
            $("#mensajenombre").text('El campo esta vacio.');
            a=1;
        }
        ...
        if (txtfechainicio>txtfechafin) {
            $("#alertfechainicio").show(0).delay(3000).hide(0);
            $("#mensajefechainicio").text('La fecha inicio no debe ser mayor a la fecha fin.');
            b=1;
        }

        if (txtfechafin<txtfechainicio) {
            $("#alertfechafin").show(0).delay(3000).hide(0);
            $("#mensajefechafin").text('La fecha fin no debe ser menor a la fecha inicio.');
            c=1;
        }

        txtestado = $('#txtestado').val();
        if (txtestado == 0) {
            $("#alertestado").show(0).delay(3000).hide(0);
            $("#mensajeestado").text('Seleccione el estado.');
            d=1;
        } else {
            if (txtestado == 3) {
                $("#alertestado").show(0).delay(3000).hide(0);
                $("#mensajeestado").text('No puede seleccionar este estado.');
                d=1;
            }
        }
        ...
        if (total==0) {
            urlbase = $("body").attr('urlbase');
            if (txtcodigo == '') {
                url = urlbase + "/configuracion/insertarParametros";
            } else {
                url = urlbase + "/configuracion/actualizarParametros";
            }
            $.ajax({
                url: url,
                method: 'POST',
                ...
                if (data.success == true) {
                    $("#alertSuccess").show(0).delay(4000).hide(0);
                    $("#mensajeSuccess").text(data.message + ' "' + nombre + '".');
                    $('#myModalVentana').modal('hide');
                    var dataTable = $('#res-config').DataTable();
                    dataTable.ajax.reload();
                } else {
                    $("#alertSuccess").show(0).delay(4000).hide(0);
                    $("#mensajeSuccess").text("SE ACTUALIZO EL CODIGO : " + codigo + ".");
                    $('#myModalVentana').modal('hide');
                    var dataTable = $('#res-config').DataTable();
                    dataTable.ajax.reload();
                }
