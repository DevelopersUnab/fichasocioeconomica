...
function consultar() {
    $.ajax({
        ...
        url: $urlbase + '/reportes/obtenerTotalFichasCalificadas',
        success: function(datos) {
            $datosNuevo = datos.data;
            if ($datosNuevo == 0) {
                $("#txttotalnopobre").val(0);
                $("#txttotalpobre").val(0);
                $("#txttotalpobreextremo").val(0);
            } else {
                for (var i in $datosNuevo) {
                    document.getElementById('txttotalnopobre').innerHTML=$datosNuevo[i].TOTALNOPOBRE;
                    document.getElementById('txttotalpobre').innerHTML=$datosNuevo[i].TOTALPOBRE;
                    document.getElementById('txttotalpobreextremo').innerHTML=$datosNuevo[i].TOTALPOBREEXTREMO;
                    document.getElementById('totalficharealizadas1').innerHTML=$datosNuevo[i].TOTALFICHASREALIZADAS;
                    document.getElementById('totalficharealizadas2').innerHTML=$datosNuevo[i].TOTALFICHASREALIZADAS;
                    document.getElementById('totalficharealizadas3').innerHTML=$datosNuevo[i].TOTALFICHASREALIZADAS;
                }
            }
        }
    });
    ...
    var dataTable = $('#res-config').DataTable({
        ...
        "columns": [{
            ...
            "render": function(data, type, row) {
                var categorizacion = row.CATEGORIZACION;
                if (categorizacion == 1) {
                    return "<label class='label label-success'>" + row.DESCRIPCIONCATEGORIZACION + "</label>"
                } else if (categorizacion == 2) {
                    return "<label class='label label-warning'>" + row.DESCRIPCIONCATEGORIZACION + "</label>"
                } else if (categorizacion == 3) {
                    return "<label class='label label-danger'>" + row.DESCRIPCIONCATEGORIZACION + "</label>"
                }
            },
            "width": 80
        }],
